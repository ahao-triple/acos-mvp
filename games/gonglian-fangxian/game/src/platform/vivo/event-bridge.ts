/**
 * vivo 触摸事件桥接 wrapper canvas（Phase 1 P0 — v4，buckets 提到 listener-store）。
 *
 ...
 *
 * v3 错误：installGlobalRoutes 在 createVivoEventBridge 里调（runtime），但 PixiJS
...
interface VivoQgTouchApi {
  onTouchStart?: (handler: (event: VivoTouchEventLike) => void) => void;
  onTouchMove?: (handler: (event: VivoTouchEventLike) => void) => void;
  onTouchEnd?: (handler: (event: VivoTouchEventLike) => void) => void;
  onTouchCancel?: (handler: (event: VivoTouchEventLike) => void) => void;
}
...
function logRawTouch(name: 'start' | 'move' | 'end' | 'cancel', event: VivoTouchEventLike): void {
  ...
  if (n <= 20 || n % 60 === 0) {
    ...
    console.log('[vivo-event-bridge] RAW touch%s #%d @ (%d, %d) touches=%d changed=%d',
...
function dispatchPointer(type: PointerType, touchEvent: VivoTouchEventLike, wrapperRef: unknown): void {
  ...
  if (dispatchCount <= 20 || dispatchCount % 60 === 0) {
    console.log('[vivo-event-bridge] dispatch %s → %s (listeners=%d) @ (%d, %d) [wrapper=%d global=%d document=%d]',
  ...
  for (const h of handlers) {
    try { h(event); } catch (e) { console.warn('[vivo-event-bridge] handler threw:', (e as Error).message); }
  }
}

function installQgTouchHook(getDispatch: () => ((type: PointerType, e: VivoTouchEventLike) => void) | null): void {
  if (touchHookInstalled) return;
  const qg = (globalThis as unknown as { qg?: VivoQgTouchApi }).qg;
  if (!qg) {
    console.warn('[vivo-event-bridge] globalThis.qg unavailable; touch input will not work');
  ...
  console.log('[vivo-event-bridge] qg touch hooks available:', JSON.stringify(hookFlags));
  try {
    qg.onTouchStart?.((e) => {
      logRawTouch('start', e);
    ...
    console.log('[vivo-event-bridge] qg.onTouchStart/Move/End/Cancel installed');
  } catch (e) {
    console.warn('[vivo-event-bridge] failed to install qg touch hook:', (e as Error).message);
  }
  touchHookInstalled = true;
}

export function createVivoEventBridge(mainCanvas: HTMLCanvasElement): HTMLCanvasElement {
  ...
  console.log('[vivo-event-bridge] sanity: globalThis.__routed=%s window.__routed=%s document.__routed=%s sameRef(g,win)=%s',
  ...
  let wrapper: unknown;
  const dispatcher = (type: PointerType, e: VivoTouchEventLike): void => dispatchPointer(type, e, wrapper);
  ...
  wrapper = {
    ...
    addEventListener(type: string, handler: unknown): void {
      ...
      if (isPointerType(type)) {
        ...
        console.log('[vivo-event-bridge] wrapper.addEventListener(%s) total=%d',
