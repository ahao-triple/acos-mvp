...
import { isVivoStrictMode } from './strict-mode';

/**
 * vivo 小游戏 runtime BOM/DOM polyfill 总入口（Phase 0 攻坚产物）。
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * 调用约定
 * ─────────────────────────────────────────────────────────────────────────────
...
(() => {
  // ----------------------------------------------------------------------
  // 严格模式短路：普通浏览器 dev（无 ?vivo-strict 标志、无 qg/tt/ks runtime）直接 return，
  // 让 PixiJS 走真原生 DOM / EventSystem，最佳表现力。本短路必须放在所有副作用之前。
  // ----------------------------------------------------------------------
  if (!isVivoStrictMode()) {
    if (typeof console !== 'undefined' && typeof console.log === 'function') {
      console.log('[vivo-polyfill] skip (not vivo-strict mode; add ?vivo-strict to URL to enable)');
    }
    return;
  }
  ...
  if (typeof nav.platform !== 'string') nav.platform = 'vivo-quickgame';
