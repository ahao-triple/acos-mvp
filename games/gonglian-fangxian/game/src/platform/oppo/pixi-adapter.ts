...
/**
 * Pixi v8 在 oppo runtime 上的 DOMAdapter（Phase 0 攻坚产物）。
 *
 * ─────────────────────────────────────────────────────────────────────────────
 ...
 * 调用约定
 * ─────────────────────────────────────────────────────────────────────────────
 *  - 必须在 `Application.init()` 之前调用 `setupOppoPixiDOMAdapter()`。
...
interface OppoQgRequestOptions {
  url: string;
  method?: 'GET' | 'POST';
  data?: unknown;
...
type OppoQgGlobal = {
  ...
  createImage?: () => unknown;
  request?: (options: OppoQgRequestOptions) => void;
};

let installed = false;

export function setupOppoPixiDOMAdapter(): void {
  if (installed) {
    return;
  }
  installed = true;
  ...
  DOMAdapter.set({
    ...
    createCanvas: ((width?: number, height?: number) => {
      ...
      throw new Error('[oppo-pixi-adapter] createCanvas: no qg.createCanvas & no document');
    ...
    createImage: (() => {
      ...
      throw new Error('[oppo-pixi-adapter] createImage: no qg.createImage & no Image ctor');
    ...
    fetch: ((url: unknown, _options?: unknown) => {
      ...
      console.log('[oppo-pixi-adapter] fetch:', reqUrl, 'isLocal:', isLocalAsset);
      if (!qg?.request) {
        return Promise.reject(new Error('[oppo-pixi-adapter] qg.request unavailable'));
      }
      const isBinary = /\.(png|jpg|jpeg|webp|bin)(\?|$)/i.test(reqUrl);
      return new Promise((resolve, reject) => {
        const timer = globalThis.setTimeout(() => {
          console.error('[oppo-pixi-adapter] fetch timeout (8s):', reqUrl);
          reject(new Error('[oppo-pixi-adapter] fetch timeout: ' + reqUrl));
        }, 8000);
        qg.request!({
          url: reqUrl,
          method: 'GET',
          ...
          success: (res) => {
            ...
            console.log('[oppo-pixi-adapter] fetch success:', reqUrl, res?.statusCode ?? 200);
            const data = res?.data;
            ...
            resolve({
              ...
              blob: () => Promise.reject(new Error('[oppo-pixi-adapter] fetch.blob not supported')),
              json: () => Promise.resolve(textValue ? JSON.parse(textValue) : null),
            } as unknown as Response);
          },
          fail: (err) => {
            globalThis.clearTimeout(timer);
            console.error('[oppo-pixi-adapter] fetch fail:', reqUrl, err);
            reject(new Error(err?.errMsg ?? '[oppo-pixi-adapter] qg.request failed'));
          },
        });
      });
  ...
  if (typeof console !== 'undefined' && typeof console.log === 'function') {
    console.log('[oppo-pixi-adapter] DOMAdapter installed (qg=%s)', qg ? 'yes' : 'no');
  }
}
