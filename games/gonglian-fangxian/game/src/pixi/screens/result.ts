...
function createOverlayBackdrop(ctx: ScreenContext): Graphics {
  ...
  g.rect(0, 0, ctx.logicalWidth, ctx.logicalHeight).fill({ color: 0x000000, alpha: 0.55 });
  return g;
}
...
export class PausedScreen implements PixiScreen {
  ...
  constructor(ctx: ScreenContext) {
    ...
    this.container.addChild(createText({ text: '暂停', size: 56, color: 0xffffff, x: 375, y: 430 }));
...
export class WonScreen implements PixiScreen {
  ...
  private readonly coinInfo: Text;
  private readonly rewardInfo: Text;
  private readonly nextLevelInfo: Text;
  private readonly doubledLabel: Text;
  private readonly doubleAdButton: ButtonHandle;
  ...
  constructor(ctx: ScreenContext) {
    ...
    this.container.addChild(createText({ text: '防线推进', size: 52, color: 0xffffff, x: 375, y: 380 }));
    this.chapterInfo = createText({ text: '', size: 26, color: 0xd1fae5, x: 375, y: 450 });
    this.coinInfo = createText({ text: '', size: 28, color: 0xfef3c7, x: 375, y: 510 });
    this.rewardInfo = createText({ text: '', size: 26, color: 0xfef3c7, x: 375, y: 560 });
    this.nextLevelInfo = createText({ text: '', size: 26, color: 0xd1fae5, x: 375, y: 610 });
    this.container.addChild(this.chapterInfo, this.coinInfo, this.rewardInfo, this.nextLevelInfo);

    this.doubleAdButton = createButton({
      x: 175, y: 670, width: 400, height: 70, label: '奖励翻倍', variant: 'ad',
      onTap: () => ctx.dispatch({ type: 'requestRewardedAd', request: { type: 'doubleWinReward' } }),
    });
    this.container.addChild(this.doubleAdButton.container);
    this.doubledLabel = createText({ text: '翻倍奖励已领取', size: 26, color: 0xd1fae5, x: 375, y: 705 });
    this.doubledLabel.visible = false;
    this.container.addChild(this.doubledLabel);

    this.container.addChild(
      createButton({ x: 175, y: 760, width: 400, height: 66, label: '下一关', variant: 'primary',
        onTap: () => ctx.dispatch({ type: 'nextLevel' }) }).container,
    );
    this.container.addChild(
      createButton({ x: 175, y: 838, width: 400, height: 66, label: '重玩本关',
        onTap: () => ctx.dispatch({ type: 'retry' }) }).container,
    );
    this.container.addChild(
      createButton({ x: 175, y: 916, width: 400, height: 66, label: '分享',
        onTap: () => ctx.dispatch({ type: 'shareReward' }) }).container,
    );
    this.container.addChild(
      createButton({ x: 175, y: 994, width: 400, height: 66, label: '返回主页',
  ...
  update(view: AppViewState): void {
    ...
    if (!s) {
      this.chapterInfo.visible = false; this.coinInfo.visible = false;
      this.rewardInfo.visible = false; this.nextLevelInfo.visible = false;
      return;
    }
    setText(this.chapterInfo, `${s.chapterTitle}  第 ${s.levelId} 关完成`);
    setText(this.coinInfo, `获得金币 ${s.baseCoins}`);
    this.chapterInfo.visible = true; this.coinInfo.visible = true;
    ...
    this.doubleAdButton.container.visible = !s.doubled;
    this.doubledLabel.visible = s.doubled;
  }
  dispose(): void { this.container.destroy({ children: true }); }
}
...
export class LostScreen implements PixiScreen {
  ...
  constructor(ctx: ScreenContext) {
    ...
    this.container.addChild(createOverlayBackdrop(ctx));
    this.container.addChild(createPanel({ x: 80, y: 300, width: 590, height: 620 }));
    this.container.addChild(createText({ text: '防线告急', size: 52, color: 0xffffff, x: 375, y: 380 }));
    this.container.addChild(createText({ text: '未完成目标', size: 28, color: 0xfef3c7, x: 375, y: 455 }));
    this.remainingText = createText({ text: '', size: 24, color: 0xd1fae5, x: 375, y: 510 });
